...
class Shard extends EventEmitter {
    ...
    createGuild(_guild) {
        ...
        if(this._client.shards.options.getAllUsers && guild.members.size < guild.memberCount) {
            this.getGuildMembers(guild.id, {
                presences: this._client.shards.options.intents && this._client.shards.options.intents & Constants.Intents.guildPresences
            });
        }
        return guild;
    }
    ...
    */
    disconnect(options = {}, error) {
        ...
        */
        ...
        if(this.sessionID && this.connectAttempts >= this._client.shards.options.maxResumeAttempts) {
            this.emit("debug", `Automatically invalidating session due to excessive resume attempts | Attempt ${this.connectAttempts}`, this.id);
            this.sessionID = null;
        }

        if(options.reconnect === "auto" && this._client.shards.options.autoreconnect) {
            /**
    ...
    */
    ...
    getGuildMembers(guildID, timeout) {
        ...
        if(this._client.shards.options.intents) {
            if(!(this._client.shards.options.intents & Constants.Intents.guildMembers)) {
    ...
    identify() {
        if(this._client.shards.options.compress && !ZlibSync) {
            /**
        ...
        if(this._client.shards.options.maxShards > 1) {
            identify.shard = [this.id, this._client.shards.options.maxShards];
        }
    ...
    onPacket(packet) {
        ...
        switch(packet.op) {
            case GatewayOPCodes.DISPATCH: {
                if(!this._client.shards.options.disableEvents[packet.t]) {
                    this.wsEvent(packet);
                }
                break;
            }
    ...
    requestGuildMembers(guildID, options) {
        ...
        if(!opts.query && !opts.user_ids && (this._client.shards.options.intents && !(this._client.shards.options.intents & Constants.Intents.guildMembers))) {
            throw new Error("Cannot request all members without guildMembers intent");
        }
        if(opts.presences && (this._client.shards.options.intents && !(this._client.shards.options.intents & Constants.Intents.guildPresences))) {
        ...
        return new Promise((res) => this.requestMembersPromise[opts.nonce] = {
            ...
            }, (options && options.timeout) || this._client.shards.options.requestTimeout)
        });
    }
    ...
    restartGuildCreateTimeout() {
        ...
        if(!this.ready) {
            ...
            this.guildCreateTimeout = setTimeout(() => {
                this.checkReady();
            }, this._client.shards.options.guildCreateTimeout);
        }
    }
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_MEMBER_UPDATE": {
                ...
                if(!(this._client.shards.options.intents & Constants.Intents.guildPresences) && packet.d.user.username !== undefined) {
                    let user = this._client.users.get(packet.d.user.id);
                    let oldUser = null;
                    if(user && (user.username !== packet.d.user.username || user.discriminator !== packet.d.user.discriminator || user.avatar !== packet.d.user.avatar)) {
                        oldUser = {
            ...
            case "GUILD_SCHEDULED_EVENT_CREATE": {
                const guild = this._client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("guildScheduledEventCreate", new GuildScheduledEvent(packet.d, this._client));
                    break;
                }
                ...
                */
                this.emit("guildScheduledEventCreate", guild.events.add(packet.d, this._client));
                break;
            }
            case "GUILD_SCHEDULED_EVENT_UPDATE": {
                const guild = this._client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("guildScheduledEventUpdate", new GuildScheduledEvent(packet.d, this._client), null);
                    break;
                }
                ...
                */
                this.emit("guildScheduledEventUpdate", guild.events.update(packet.d, this._client), oldEvent);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_DELETE": {
                const guild = this._client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("guildScheduledEventDelete", new GuildScheduledEvent(packet.d, this._client));
                    break;
                }
                /**
                ...
                */
                this.emit("guildScheduledEventDelete", guild.events.remove(packet.d) || new GuildScheduledEvent(packet.d, this._client));
                break;
            }
